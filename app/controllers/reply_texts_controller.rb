class ReplyTextsController < ApplicationController
  ...
  def index
    ...
    @reply_texts = ReplyText.order("updated_at desc").page([params[:page].to_i,1].max).per(@per_page)
  ...
  def show
    @reply_text = ReplyText.find(params[:id])
  ...
  def create
    @reply_text = ReplyText.new(params[:reply_text])
